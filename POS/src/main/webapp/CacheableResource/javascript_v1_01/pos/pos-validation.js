...
jQuery.validator.addMethod("idNo", function(value, element, param) {
	...
	if(param) {
		...
		var idCarErrorCode;
		var idCarErrorReason;
		if(target && val) {
			if($(target).val() == val) {
				idCarError=Zxb.check.checkCnId(value.toUpperCase()).split(",")[0];
				valid = value && (idCarError == "0");
				if(!valid) {
				idCarErrorReason=Zxb.check.checkCnId(value.toUpperCase()).split(",")[1];
						this.showLabel(element, idCarErrorReason);
						return true;				
...
jQuery.validator.addMethod("smallerThan2", function(value, element,param) {	
	  return value=="" || param=="" || parseFloat(value) < parseFloat(param);
}, "只能小于原值" );
//不能大于原值,这里仅限于数字比较
