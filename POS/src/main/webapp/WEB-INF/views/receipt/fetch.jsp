...
<sf:override name="pathString">保单回执管理&gt;&gt;回销取单</sf:override>
<script type="text/javascript">
$(function(){
	$("form").validate();
	
	$("#queryBt, #nextBt").click(function(){
		$("#fetchType").val($(this).attr("name"));
		if($("#fetchType").val()=="P" && $.trim($("#policyNo").val())==""){
...
<table class="layouttable">
  <tr>
    ...
    <td class="layouttable_td_widget">
      <input type="text" name="policyNo" id="policyNo" <c:if test="${seniorBizFlag eq 'N'}">onpaste="return false" ondragenter="return false"</c:if> />
      再输入一次：<input type="text" name="policyNoRepeat" class="partValidate {equalTo:'#policyNo'}" <c:if test="${seniorBizFlag eq 'N'}">onpaste="return false" ondragenter="return false"</c:if> />
    </td>
    <td align="right">
    	<a class="easyui-linkbutton" href="javascript:void(0)" iconCls="icon-search" id="queryBt" name="P">查询</a>
    </td>
  </tr>
  <tr>
    <td class="layouttable_td_label">保单渠道：</td>
    <td class="layouttable_td_widget">
    	<select name="channel">
    ...
    <td align="right">
        <a class="easyui-linkbutton" href="javascript:void(0)" iconCls="icon-ok" id="nextBt" name="N">取下一单</a>
    </td>
  </tr>
</table>
</div>
