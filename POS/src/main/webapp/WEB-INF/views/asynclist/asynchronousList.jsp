...
<sf:override name="pathString">异步清单报表&gt;&gt;异步清单报表任务查询</sf:override>
...
function queryReportTask(){
	...
	$("#reportTask").datagrid({
		title:"已提交报表任务列表",
		url:"${ctx}async/queryReportTaskPage.do?"+$(form).formSerialize(),
		pagination:true,
		rownumbers:true,
		singleSelect:true,
		striped:true,
		...
		columns:[[{field:"USER_ID", title:"提交人", align : 'center', width:wd*20/100},
		         ...
		         {field:"PROC_STATUS", title:"处理状态", align : 'center', width:wd*15/100, formatter:function(value, rowData, rowIndex) {
		        	 if(rowData.PROC_STATUS == '处理失败') {
		        		 return "处理失败<a class='easyui-linkbutton' href=\"javascript:$.messager.alert('失败原因','"+rowData.REMARK+"')\" iconCls='icon-print'>(失败原因)</a>";
		        	 } else {
		        		 return rowData.PROC_STATUS;
		        	 }
		         }},
		         {field:"DATA_AMOUNT", title:"数据量", align : 'center', width:wd*5/100},
		         ...
		         {field:"IM_FILE_ID", title:"操作", align : 'center', width:wd*10/100, formatter:function(value, rowData, rowIndex){
		        	 if(rowData.IM_FILE_ID==null||rowData.IM_FILE_ID==''){
			        	 ...
			        	 return "<a class='easyui-linkbutton' href='${ctx}async/downLoadEexelData.do?imFileId="+rowData.IM_FILE_ID+"&userId="+rowData.USER_ID+"&startDate="+startDate +"&endDate="+endDate +"' iconCls='icon-print'>下载</a>"
...
function updatePosListControlIsValid(listCode,userId,submitTime){
	$.messager.confirm('确认作废', '您确认要作废此条记录吗？', function(r){
		if (r){
			$.ajax({type: "GET",
				url:"${ctx}async/updatePosListControlIsValid.do?listCode="+listCode+"&userId="+userId,
